...
## V41 — диагностика VR BOX 3.0

Добавлено отдельное окно «ПРОВЕРИТЬ VR BOX 3.0». Оно не меняет AR/браузерный конвейер.

- GameController: количество подключённых контроллеров и их имена.
- Live Input: стики и кнопки стандартного GameController.
- Motion / Gyro: данные движения, если контроллер сам предоставляет GCMotion.
- HID press/change/release: попытка поймать клавиатурные/HID-события старого VR-пульта.
- Журнал событий в реальном времени.

Цель теста — установить, какие события iOS реально отдаёт конкретному VR BOX 3.0, прежде чем привязывать его к виртуальной руке.


## V50 — VR Desktop и VR-приложения

Главный экран внутри VR — отдельная стереопанель с домашним экраном, сеткой приложений, dock, поисковой строкой и локальными VR-экранами. Веб-сервисы остаются внутри WKWebView. Локальные оболочки добавлены для Фото, Файлов, Сообщений, Телефона, Заметок, Калькулятора и Настроек.

## V50 — Controller Hand

Добавлен мост GameController. При наличии совместимого профиля стик двигает виртуальную 3D-кисть по панели, A нажимает, B возвращает назад, X открывает Home, Y центрирует, Menu выходит из VR. Если iOS предоставляет GCMotion, сервис также диагностирует наличие motion-профиля; сигналы гироскопа не выдумываются при отсутствии профиля.

## V57 — VR Arcade

Добавлен раздел `Игры` в VR Desktop. Встроены Fruit Slice VR, Neon Dodge и Stack Rush. Игры работают через pointer events, включая hover/pointermove без pinch, чтобы положение пальца отслеживалось в реальном времени.
